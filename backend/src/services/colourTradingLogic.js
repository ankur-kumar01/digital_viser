...
class ColourTradingLogic {
  ...
  async calculateResult() {
    ...
    try {
      ...
      const [bets] = await conn.query('SELECT color, bet_amount FROM ct_bets WHERE round_id = ?', [this.roundId]);
      
      // Calculate liability for each possible winning number 0-9
      const getLiabilityForNumber = (n) => {
        let totalPay = 0;
        bets.forEach(b => {
          const select = b.color;
          ...
          if (select === n.toString()) {
            ...
            else if (n === 0) totalPay += amt * 1.5;
          }
          else if (select === 'green') {
            if ([1, 3, 7, 9].includes(n)) totalPay += amt * 2;
            else if (n === 5) totalPay += amt * 1.5;
          }
          else if (select === 'violet') {
            if (n === 0 || n === 5) totalPay += amt * 4.5;
          }
        });
        return totalPay;
      };
      ...
      let finalColor = 'red';
      if (finalNumber === 0) finalColor = 'violet';
      else if (finalNumber === 5) finalColor = 'violet';
      ...
      // 4. Resolve Bets
      const [activeBets] = await conn.query('SELECT * FROM ct_bets WHERE round_id = ? AND status = "active"', [this.roundId]);
      
      for (const bet of activeBets) {
        let isWin = false;
        let mult = 0;
        
        const select = bet.color;
        ...
        if (isWin) {
          ...
          await conn.query('UPDATE users SET balance = balance + ? WHERE id = ?', [winAmount, bet.user_id]);
          ...
          await conn.query('UPDATE ct_bets SET status = "won", win_amount = ? WHERE id = ?', [winAmount, bet.id]);
        } else {
          await conn.query('UPDATE ct_bets SET status = "lost", win_amount = 0 WHERE id = ?', [bet.id]);
        }
      }
