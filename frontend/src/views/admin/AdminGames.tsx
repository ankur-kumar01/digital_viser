...
export const AdminGames: React.FC = () => {
  ...
  const [fsHouseEdge, setFsHouseEdge] = useState('5');
  ...
  const fetchGames = async () => {
    try {
      ...
      if (settingsRes.fruit_slasher_house_edge) {
        setFsHouseEdge(settingsRes.fruit_slasher_house_edge);
      }
      if (analyticsRes) {
  ...
  const handleSaveSettings = async (game: string) => {
    ...
    try {
      ...
      } else if (game === 'fruit-slasher') {
        await adminAPI.updateSettings({ fruit_slasher_house_edge: fsHouseEdge });
      }
  ...
  return (
    <div className="fade-in">
            ...
            <div key={g.id} className="glass-card" style={{ display: 'flex', flexDirection: 'column', padding: '0', overflow: 'hidden', border: g.is_active ? '1px solid var(--accent-primary-glow)' : '1px solid var(--border-glass)' }}>
              ...
              <div style={{ padding: '20px', flex: 1, display: 'flex', flexDirection: 'column' }}>
                ...
                <div style={{ display: 'flex', gap: '10px' }}>
                  ...
                  {g.slug === 'fruit-slasher' && (
                    <button 
                      onClick={() => setSettingsModalOpen('fruit-slasher')}
                      className="btn btn-secondary"
                      style={{ padding: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                      title="Fruit Slasher Settings"
                    >
                      <Settings size={18} />
                    </button>
                  )}
      ...
      {/* Premium Fruit Slasher Settings Modal */}
      {settingsModalOpen === 'fruit-slasher' && (
        <div 
          style={{
            position: 'fixed', top: 0, left: 0, right: 0, bottom: 0,
            background: 'rgba(0, 0, 0, 0.75)',
            backdropFilter: 'blur(12px)',
            zIndex: 1000,
            display: 'flex', alignItems: 'center', justifyEncoding: 'center', justifyContent: 'center',
            animation: 'fadeIn 0.3s ease-out'
          }}
        >
          <div 
          ...
          >
            ...
            <div style={{ padding: '32px', position: 'relative', zIndex: 1 }}>
              <div style={{ background: 'rgba(0,0,0,0.2)', padding: '24px', borderRadius: '16px', border: '1px solid var(--border-glass)' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                  <div style={{ position: 'relative', flex: '0 0 120px' }}>
                    <input 
                      type="number" 
                      value={fsHouseEdge} 
                      onChange={(e) => setFsHouseEdge(e.target.value)}
                      style={{
                        width: '100%',
                        background: 'var(--bg-primary)',
                        border: '2px solid var(--border-light)',
                ...
                <div style={{ marginTop: '20px', padding: '12px', background: 'var(--accent-primary-glow)', borderRadius: '8px', borderLeft: '3px solid var(--accent-primary)' }}>
                  <p style={{ margin: 0, fontSize: '0.8rem', color: 'var(--text-secondary)', lineHeight: 1.5 }}>
                    <strong style={{ color: 'var(--accent-primary)' }}>Notice:</strong> The house edge determines the pre-calculated crash limits. A house edge of 5% means the game will limit players to win back ~95% of overall wagers.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div style={{ padding: '24px 32px', background: 'var(--bg-tertiary)', borderTop: '1px solid var(--border-light)', display: 'flex', justifyContent: 'flex-end', gap: '12px', position: 'relative', zIndex: 1 }}>
              ...
              <button 
                onClick={() => handleSaveSettings('fruit-slasher')} 
                disabled={savingSettings}
                style={{
                  padding: '12px 24px',
                  background: 'var(--accent-primary)',
